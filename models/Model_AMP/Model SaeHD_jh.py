...
""").lower()
    ...
    def onGetPreview(self, samples):
        ...
        if self.resolution <= 384:
            ...
            result += [ ((f"src-src,dst-dst,prediction. Model Info: SAEHD {self.resolution} Resolution {self.options['archi'].upper()}"), np.concatenate (st, axis=0 )), ]

            wt = []
            for i in range(n_samples):
                ar = SW[i], SS[i], DW[i], DD[i], SD[i]
                wt.append ( np.concatenate ( ar, axis=1) )
            result += [ ((f"degraded src-src,degraded dst-dst,prediction. Model Info: SAEHD {self.resolution} Resolution {self.options['archi'].upper()}"), np.concatenate (wt, axis=0 )), ]
            ...
            result += [ ((f"masked src-src,masked dst-dst,masked prediction. Model Info: SAEHD {self.resolution} Resolution {self.options['archi'].upper()}"), np.concatenate (st_m, axis=0 )), ]
        else:
            result = []
