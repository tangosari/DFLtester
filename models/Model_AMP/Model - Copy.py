...
class AMPModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        uniform_yaw = self.options['uniform_yaw']
    ...
    def onGetPreview(self, samples):
        ...
        n_samples = min(4, self.get_batch_size(), 1200 // self.resolution )
        ...
        if pretrain:
            ...
            st += [ np.concatenate ((SW[i], DD[i], SD_050[i] ), axis=1) ]

            result += [ ((f"AMP morph 0.50. Model Info: AMP {self.resolution} Resolution"), np.concatenate (st, axis=0 )), ]
        else:
            st =  [ np.concatenate ((S[i],  D[i],  DD[i]*DDM_000[i]), axis=1) ]
            st += [ np.concatenate ((SS[i], DD[i], SD_050[i] ), axis=1) ]
            result += [ ((f"AMP morph 0.50. Model Info: AMP {self.resolution} Resolution"), np.concatenate (st, axis=0 )), ]

        st =  [ np.concatenate ((DD[i], SD_025[i],  SD_050[i]), axis=1) ]
        st += [ np.concatenate ((SD_065[i], SD_075[i], SD_100[i]), axis=1) ]
        result += [ ((f"Morph list. Model Info: AMP {self.resolution} Resolution"), np.concatenate (st, axis=0 )), ]
        ...
        result += [ ((f"Morph list masked. Model Info: AMP {self.resolution} Resolution"), np.concatenate (st, axis=0 )), ]
        
        st =  [ np.concatenate ((S[i], D[i], SD_050[i]), axis=1) ]
        st += [ np.concatenate ((SW[i], DW[i], SD_100[i] ), axis=1) ]        
        result += [ ((f"src-degraded src,dst-degraded dst,src>dst %50 morph / src>dst %100 morph. Model Info: AMP {self.resolution} Resolution"), np.concatenate (st, axis=0 )), ]
